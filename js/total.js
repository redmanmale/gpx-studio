...
`;
            ...
            if (!mergeAll || this.traces.length == 1 || trace_idx !== undefined) {
                output.push({
                    ...
                    text: (xmlStart1+xmlStart2+this.encodeString(this.traces[i].name)+xmlStart3+waypointsOutput+xmlOutput+xmlEnd2)
                });
                xmlOutput = '';
                waypointsOutput = '';
            }
        ...
        if (mergeAll && this.traces.length > 1) {
            output.push({
                name: 'track.gpx',
                text: (xmlStart1+xmlStart2+'track'+xmlStart3+waypointsOutput+xmlOutput+xmlEnd2)
            });
        }
